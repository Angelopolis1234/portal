...
app.get('/addEmpleado', (req, res) => {
	...
	if (data.password === password) {
		const sql = `INSERT INTO empleado(nombre,apellido_paterno,apellido_materno,direccion,telefono,correo,salario,cargo,fecha_ingreso,id_horario) VALUES (\'${data.nombre}\',\'${data.apellidop}\',\'${data.apellidom}\',\'${data.direccion}\',\'${data.telefono}\',\'${data.correo}\',${data.salario},\'${data.cargo}\',\'${data.decha}\',${data.horario})`
		console.log(`[API] :: QUERY -> ${sql}`);
		connection.query(sql, (err, results) => {
			if (err) throw err;
